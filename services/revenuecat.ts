...
async function getPurchases(): Promise<typeof import('react-native-purchases') | null> {
...
export async function configureRevenueCat(): Promise<void> {
  ...
  try {
    ...
    if (apiKey.startsWith('appl_YOUR_') || apiKey.startsWith('goog_YOUR_')) return;
...
export async function getPremiumState(): Promise<PremiumState> {
  ...
  try {
    ...
    const weeklyPkg = current.weekly ?? current.availablePackages.find((p: { packageType: string }) => p.packageType === 'WEEKLY') ?? null;
    const lifetimePkg = current.lifetime ?? current.availablePackages.find((p: { packageType: string }) => p.packageType === 'LIFETIME') ?? null;

    const weeklyPrice = weeklyPkg?.product?.priceString ?? fallback.weeklyPrice;
    const lifetimePrice = lifetimePkg?.product?.priceString ?? fallback.lifetimePrice;
...
export async function purchaseWeekly(): Promise<{ success: boolean }> {
  ...
  try {
    await configureRevenueCat();
    const offerings = await Purchases.getOfferings();
    const pkg = offerings.current?.weekly ?? offerings.current?.availablePackages?.find((p: { packageType: string }) => p.packageType === 'WEEKLY');
    if (!pkg) {
      console.warn('Weekly package not found');
      return { success: false };
    }
    const result = await Purchases.purchasePackage(pkg);
    const isPremium = result.customerInfo && Object.keys(result.customerInfo.entitlements.active).length > 0;
...
export async function purchaseLifetime(): Promise<{ success: boolean }> {
  ...
  try {
    await configureRevenueCat();
    const offerings = await Purchases.getOfferings();
    const pkg = offerings.current?.lifetime ?? offerings.current?.availablePackages?.find((p: { packageType: string }) => p.packageType === 'LIFETIME');
    if (!pkg) {
