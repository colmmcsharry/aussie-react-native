  ...
  const latestList = useMemo((): LatestItem[] => {
    ...
    youtubeVideos.forEach((entry) => {
      list.push({
        type: "youtube",
        entry,
        sortKey: getSortableTimestamp(entry.date),
      });
    });
  ...
  const renderLatestItem = useCallback(
    ({ item }: { item: LatestItem }) => {
      ...
      const isPremiumVideo =
        teacherKey != null && PREMIUM_TEACHER_KEYS.includes(teacherKey);
    ...
    <View style={[styles.container, { backgroundColor: ContentBg }]}>
      ...
      <View style={styles.toggleRow}>
        <Pressable
          style={[
            styles.toggleBtn,
            viewMode === "latest" && styles.toggleBtnActive,
          ]}
        ...
        >
          <ThemedText
            style={[
              styles.toggleText,
              viewMode === "latest" && styles.toggleTextActive,
            ]}
          >
            Latest
          </ThemedText>
        ...
        <Pressable
          style={[
            styles.toggleBtn,
            viewMode === "teachers" && styles.toggleBtnActive,
          ]}
        ...
        >
          <ThemedText
            style={[
              styles.toggleText,
              viewMode === "teachers" && styles.toggleTextActive,
            ]}
          >
            Teachers
          </ThemedText>
      ...
      {viewMode === "latest" ? (
        <FlatList
          ...
          keyExtractor={(item) =>
            item.type === "youtube"
              ? item.entry.youtubeId
              : getVideoId(item.video)
          }
        ...
        >
          {(() => {
            const staticKeys = TEACHER_KEYS.filter((k) => {
              ...
              const gistForTeacher = (youtubeVideos ?? []).filter(
                (e) => e.teacher === k,
              );
            ...
            const dynamicFromGist = [
              ...
              (k) =>
                !TEACHER_KEYS.includes(k as (typeof TEACHER_KEYS)[number]) &&
                !dynamicFromVimeo.includes(k),
            );
            const allKeys = [
              ...staticKeys,
              ...dynamicFromVimeo,
            ...
            return allKeys.map((key) => {
              ...
              if (teacherVideos.length > 0) {
                let thumbVideo = profile.thumbnailFromVimeoId
                  ? teacherVideos.find(
                      (v) => getVideoId(v) === profile.thumbnailFromVimeoId,
                    )
                  : null;
                    ...
                    <View style={styles.teacherThumbWrap}>
                      {thumbnailUri ? (
                        ...
                        <View
                          style={[
                            styles.teacherThumb,
                            styles.teacherThumbPlaceholder,
                          ]}
                        >
                      ...
                      <View style={styles.teacherVideosRow}>
                        <ThemedText style={styles.teacherVideosCount}>
                          {teacherVideos.length +
                            (profile.youtubeVideos?.length ?? 0) +
                            gistForTeacher.length}{" "}
                          video
                          {teacherVideos.length +
                            (profile.youtubeVideos?.length ?? 0) +
                            gistForTeacher.length ===
